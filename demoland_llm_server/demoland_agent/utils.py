...
def load_scenario(file, baseline=None):
    ...
    scenario_df = scenario_df.rename(columns={"air_quality": "air_pollution_index"})
    scenario_df.signature_type =scenario_df.signature_type.apply(lambda x: sig_lookup[x])
    if(baseline is not None):
        diff_cols = ["air_pollution_index", "house_price", "job_accessibility", "greenspace_accessibility"] 	
